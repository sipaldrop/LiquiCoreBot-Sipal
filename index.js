...
const config = {
    ...
    duelMinInterval: 1 * 60 * 1000,  // 1 minute min
    duelMaxInterval: 5 * 60 * 1000,  // 5 minutes max
...
function renderDashboard() {
    ...
    const table = new Table({
        ...
        colWidths: [10, 9, 8, 10, 11, 11, 10, 12, 12]
    });
...
async function depositVault(wallet, type, idx) {
    ...
    const buffer = ethers.utils.parseUnits("550", decimals);
...
async function processDuel(account, idx) {
    ...
    // 2. Deposit max to vault for advantage
    await depositVault(wallet, 1, idx);
    await depositVault(wallet, 0, idx);
    
    // 3. Try to accept open duels
    ...
    // 4. If no duels to accept, create one
    if (!accepted && openDuels.length === 0) {
    ...
    state[idx].duelStatus = accepted ? '✅' : '⏳';
    state[idx].lastDuel = Date.now();
    state[idx].nextDuel = new Date(Date.now() + randomDelay(config.duelMinInterval, config.duelMaxInterval));
}
...
async function main() {
    accounts.forEach((_, i) => {
        ...
        state[i].nextDaily = getNextDailySchedule();
